...
def handle_label_loan_status(event_data: dict, labels: list, self_label: int) -> dict:
    ...
    for loan in result:
        ...
        message += f"Ընդհանուր մնացորդ: {loan['TotalSum']}\n"
...
def handle_label_pledge_question(event_data: dict, labels: list, self_label: int) -> dict:
    message: str = "gրավի մասին տեղեկություն ստանալու համար կարող եք այցելել մեր կայք՝ https://www.fastbank.am/ոսկու-գրավով-վարկեր ։ Սեղմելով ներքևում գտնվող կոճակը, մեր աջակցման խումբը կկապնվի ձեզ հետ։"
    buttons = {
        'contact_me': {
            'label': [
...
def handle_label_arrive(event_data: dict, labels: list, self_label: int) -> dict:
    ...
    if 9 in labels:
        message: str = ""
    #TODO: check if user has order tell when user can arrive to take his order
    #if self_label and 11 in labels:
    return utils.format_return_label_data(message)
...
def handle_label_terminal_related(event_data: dict, labels: list, self_label: int) -> dict:
    ...
    buttons = {
        'contact_me': {
            'label': [
                {"display_name": "Կապնվեք ինձ հետ (Տերմինալ)", "return_value": self_label}
            ],
        }
    }
    return utils.format_return_label_data(message, buttons)
...
def handle_label_proposal(event_data: dict, labels: list, self_label: int) -> dict:
    ...
    message: str = (
        "Ձեր առաջարկը կուղարկվի մեր մասնագետին: Եթե մեզ հետաքրքրի, մենք կկապվենք ձեր հետ"
    )
    return utils.format_return_label_data(message)
